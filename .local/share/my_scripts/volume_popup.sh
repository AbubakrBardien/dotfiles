  ...
  mute)
    ...
    if [[ $2 != "waybar" ]]; then # When clicking on the waybar volume module, don't notify
        if [[ $isMuted = "[MUTED]" ]]; then # If Muted
            dunstify -a "volume_popup" -u low -h string:x-dunst-stack-tag:vol_tag -i "$HOME/.config/dunst/icons/mute.png" "Muted" -t $timeout
        else
            dunstify -a "volume_popup" -u low -h string:x-dunst-stack-tag:vol_tag -i "$HOME/.config/dunst/icons/volume.png" "Volume: $volume%" -t $timeout
        fi
    fi
    ;;
esac
