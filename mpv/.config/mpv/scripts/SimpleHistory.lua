...
function list_sort(tab, sort)
	...
	elseif sort == "alphanum-asc" or sort == "alphanum-desc" then
		...
		if sort == "alphanum-asc" then
			table.sort(tab, function(a, b)
				return tostring(a["found_path"]):gsub("%.?%d+", padnum) .. ("%3d"):format(#b) > tostring(b["found_path"]):gsub("%.?%d+", padnum) .. ("%3d"):format(#a)
			end)
...
function parse_header(string)
	...
	if filterName ~= "all" then
		string = string:gsub("%%filter%%", filterName):gsub("%%prefilter%%", o.header_filter_pre_text):gsub("%%afterfilter%%", o.header_filter_after_text)
	else
	...
	if list_total_duration > 0 then
		string = string:gsub("%%prelistduration%%", o.header_list_duration_pre_text):gsub("%%afterlistduration%%", o.header_list_duration_after_text)
	else
	...
	if list_total_length > 0 then
		string = string:gsub("%%prelistlength%%", o.header_list_length_pre_text):gsub("%%afterlistlength%%", o.header_list_length_after_text)
	else
	...
	if list_total_remaining > 0 then
		string = string:gsub("%%prelistremaining%%", o.header_list_remaining_pre_text):gsub("%%afterlistremaining%%", o.header_list_remaining_after_text)
	else
		string = string:gsub("%%prelistremaining%%", ""):gsub("%%afterlistremaining%%", "")
	end

	if #list_highlight_cursor > 0 then
		string = string:gsub("%%highlight%%", #list_highlight_cursor):gsub("%%prehighlight%%", o.header_highlight_pre_text):gsub("%%afterhighlight%%", o.header_highlight_after_text)
	else
		string = string:gsub("%%highlight%%", ""):gsub("%%prehighlight%%", ""):gsub("%%afterhighlight%%", "")
	end

	if sortName and sortName ~= o.header_sort_hide_text then
		string = string:gsub("%%sort%%", sortName):gsub("%%presort%%", o.header_sort_pre_text):gsub("%%aftersort%%", o.header_sort_after_text)
	else
	...
	if search_active then
		...
		string = string:gsub("%%search%%", osd_search_color .. search_string_osd .. osd_header_color):gsub("%%presearch%%", o.header_search_pre_text):gsub("%%aftersearch%%", o.header_search_after_text)
	else
...
function get_list_keybinds()
	...
	for i = 1, #o.list_highlight_move_keybind do
		for j = 1, #o.list_move_up_keybind do
			mp.add_forced_key_binding(o.list_highlight_move_keybind[i] .. "+" .. o.list_move_up_keybind[j], "highlight-move-up" .. j, function()
				list_move_up("highlight")
			end, "repeatable")
		end
		for j = 1, #o.list_move_down_keybind do
			mp.add_forced_key_binding(o.list_highlight_move_keybind[i] .. "+" .. o.list_move_down_keybind[j], "highlight-move-down" .. j, function()
				list_move_down("highlight")
			end, "repeatable")
		end
		for j = 1, #o.list_move_first_keybind do
			mp.add_forced_key_binding(o.list_highlight_move_keybind[i] .. "+" .. o.list_move_first_keybind[j], "highlight-move-first" .. j, function()
				list_move_first("highlight")
			end, "repeatable")
		end
		for j = 1, #o.list_move_last_keybind do
			mp.add_forced_key_binding(o.list_highlight_move_keybind[i] .. "+" .. o.list_move_last_keybind[j], "highlight-move-last" .. j, function()
				list_move_last("highlight")
			end, "repeatable")
		end
		for j = 1, #o.list_page_up_keybind do
			mp.add_forced_key_binding(o.list_highlight_move_keybind[i] .. "+" .. o.list_page_up_keybind[j], "highlight-page-up" .. j, function()
				list_page_up("highlight")
			end, "repeatable")
		end
		for j = 1, #o.list_page_down_keybind do
			mp.add_forced_key_binding(o.list_highlight_move_keybind[i] .. "+" .. o.list_page_down_keybind[j], "highlight-page-down" .. j, function()
...
function history_blacklist_check()
	if o.history_blacklist == nil then
		msg.warn("ignoring history_blacklist option due to an inappropriate value")
		return false
	end --1.1.7# fix crash of history_blacklist if given
	...
	elseif not starts_protocol(protocols, filePath) then
		if has_value(o.history_blacklist, filePath:match("^(.-)([^\\/]-)%.([^\\/%.]-)%.?$"), nil) or has_value(o.history_blacklist, filePath:match("^(.-)([^\\/]-)%.([^\\/%.]-)%.?$"):gsub("\\$", ""), nil) then
			msg.info(blacklist_msg)
			return invertable_return[1]
		elseif has_value(o.history_blacklist, filePath:match("%.([^%.]+)$"), nil) or has_value(o.history_blacklist, "." .. filePath:match("%.([^%.]+)$"), nil) then
			msg.info(blacklist_msg)
			return invertable_return[1]
		else
	...
	elseif starts_protocol(protocols, filePath) then
		if has_value(o.history_blacklist, filePath:match("(.-)(:)"), nil) or has_value(o.history_blacklist, filePath:match("(.-:)"), nil) or has_value(o.history_blacklist, filePath:match("(.-:/?/?)"), nil) then
			msg.info(blacklist_msg)
			return invertable_return[1]
		elseif filePath:find("https?://") == 1 then
			...
			for i = 1, #different_checks do
				if different_checks[i] and has_value(o.history_blacklist, different_checks[i], nil) or different_checks[i] .. "/" and has_value(o.history_blacklist, different_checks[i] .. "/", nil) then
					msg.info(blacklist_msg)
					return invertable_return[1]
				end
			end
...
mp.observe_property("idle-active", "bool", function(_, v)
	...
	if v and o.auto_run_incognito_mode and not incognito_auto_run_triggered or not v and o.auto_run_incognito_mode and not incognito_auto_run_triggered then
...
-- Credit to Eisa01 for the script: https://github.com/Eisa01/mpv-scripts
