...
--          so the system's Subliminal location needs to be specified:
...
function should_download_subs_in(language)
	for i, track in ipairs(sub_tracks) do
		...
		elseif track["lang"] == language[3] or track["lang"] == language[2] or (track["title"] and track["title"]:lower():find(language[3])) then
...
-- Credit to David Deprost for the script: https://github.com/davidde/mpv-autosub
